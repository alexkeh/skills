...
- Use `node tools/apexctl.mjs runtime validate --app-path <absolute_app_path> --db-connection-name <db_connection_name> --apex-root <resolved_build_root>` as the public check-only gate for generated apps.
- Treat live/compiler-driven target-build evidence and VSCode Problems diagnostics as required validation sources. Missing or failing evidence records `VALIDATION_DUAL_SOURCE_REQUIRED_001` and blocks completion.
- Use emitted `problems.json` as the validation review interface; patch only reported problems and rerun `runtime validate`.
